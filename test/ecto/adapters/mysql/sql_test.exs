defmodule Ecto.Adapters.Mysql.SQLTest do
  ...
  test "from without model" do
    ...
    assert SQL.select(query) == {~s{SELECT p0.`x`\nFROM `posts` AS p0}, []}
  end

  test "select" do
    query = Model |> select([r], {r.x, r.y}) |> normalize
    assert SQL.select(query) == {~s{SELECT m0.`x`, m0.`y`\nFROM `model` AS m0}, []}

    query = Model |> select([r], {r.x, r.y + 123}) |> normalize
    assert SQL.select(query) == {~s{SELECT m0.`x`, m0.`y` + 123\nFROM `model` AS m0}, []}
  end

  test "distinct" do
    query = Model |> distinct([r], r.x) |> select([r], {r.x, r.y}) |> normalize
    assert SQL.select(query) == {~s{SELECT DISTINCT ON (m0.`x`) m0.`x`, m0.`y`\nFROM `model` AS m0}, []}

    query = Model |> distinct([r], 2 * 2) |> select([r], r.x) |> normalize
    assert SQL.select(query) == {~s{SELECT DISTINCT ON (2 * 2) m0.`x`\nFROM `model` AS m0}, []}

    query = Model |> distinct([r], [r.x, r.y]) |> select([r], {r.x, r.y}) |> normalize
    assert SQL.select(query) == {~s{SELECT DISTINCT ON (m0.`x`, m0.`y`) m0.`x`, m0.`y`\nFROM `model` AS m0}, []}
  end
  ...
  test "binary ops" do
    ...
    assert SQL.select(query) == {~s{SELECT m0.`x` = 2\nFROM `model` AS m0}, []}
    ...
    assert SQL.select(query) == {~s{SELECT m0.`x` AND FALSE\nFROM `model` AS m0}, []}
  ...
  test "interpolated values" do
    ...
    SELECT ?
    FROM `model` AS m0
    INNER JOIN `model2` AS m1 ON ?
    INNER JOIN `model2` AS m2 ON ?
    WHERE (?) AND (?)
    GROUP BY ?, ?
    HAVING (?) AND (?)
    ORDER BY ?, ?
  ...
  test "nested expressions" do
    ...
    assert SQL.select(query) == {~s{SELECT (m0.`x` + (m0.`y` + ?)) - 3\nFROM `model` AS m0}, [-123]}
  end
  ...
  test "update all" do
    ...
    query = Model |> Queryable.to_query |> normalize
    assert SQL.update_all(query, [x: quote do: &0.x + 1], %{}) ==
           {~s{UPDATE `model` AS m0\nSET `x` = m0.`x` + 1}, []}
  ...
  test "in expression" do
    ...
    assert SQL.select(query) == {~s{SELECT m0.`x` BETWEEN 1 AND 3\nFROM `model` AS m0}, []}

    query = Model |> select([e], 1 in 1..(e.x + 5)) |> normalize
    assert SQL.select(query) == {~s{SELECT 1 BETWEEN 1 AND m0.`x` + 5\nFROM `model` AS m0}, []}
  end
  ...
  test "functions" do
    ...
    assert SQL.select(query) == {~s{SELECT random()\nFROM `model` AS m0}, []}

    query = Model |> select([], round(12.34)) |> normalize
    assert SQL.select(query) == {~s{SELECT round(12.34)\nFROM `model` AS m0}, []}

    query = Model |> select([], round(12.34, 1)) |> normalize
    assert SQL.select(query) == {~s{SELECT round(12.34, 1)\nFROM `model` AS m0}, []}

    query = Model |> select([], pow(7, 2)) |> normalize
    assert SQL.select(query) == {~s{SELECT 7 ^ 2\nFROM `model` AS m0}, []}
  end
